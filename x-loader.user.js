...
(function() {
    ...
    // ─── Re-find the active button wrapper fresh from DOM ───
    function getActiveWrapper() {
        return document.querySelector('[data-x-feature="ai-captions"][data-active="true"]');
    }

    function getPlayerFromWrapper(w) {
        if (!w) return null;
    ...
    function activate(w) {
        ...
        if (captionsData) {
            // Find player fresh — wrapper might have been re-rendered
            var fresh = getActiveWrapper();
            if (fresh) startCaptionDisplay(fresh);
            return;
        }

        var tweetUrl = document.querySelector('link[rel="canonical"]');
        tweetUrl = tweetUrl ? tweetUrl.href : location.href.split('?')[0].split('#')[0];
        showStatus(w, 'Transcribing...');
        console.log('[X] Requesting:', tweetUrl);
        ...
        GM_xmlhttpRequest({
            ...
            onerror: function() { console.error('[X] Server down'); }
        });
    }

    // ─── Poll without wrapper reference — re-find active button ───
    function pollJob(jobId) {
        var attempts = 0;
        function poll() {
            if (!getActiveWrapper()) return; // button was deactivated or detached
            attempts++;
            GM_xmlhttpRequest({
                method: 'POST',
                ...
                onload: function(r) {
                    ...
                    try {
                        ...
                        if (resp.status === 'done') {
                            ...
                            hideOverlay(w);
                            startCaptionDisplay(w);
                        } else if (resp.status === 'error') {
                            showStatus(w, 'Error');
                        } else {
                            showStatus(w, 'Transcribing... (' + attempts + 's)');
    ...
    function startCaptionDisplay(w) {
        hideOverlay(w);
        if (!captionsData) return;
        var p = getPlayerFromWrapper(w);
        if (!p) return;
        var v = p.querySelector('video');
        if (!v) { console.log('[X] No video element'); return; }
    ...
    function tryProcess() {
        ...
        if (!ok && retries < 40) { retries++; setTimeout(tryProcess, 500); }
