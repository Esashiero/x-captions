// ==UserScript==
// @name         X.com AI Captions (Browser-Only)
// @namespace    local.x-features
// @version      3.2
// @description  AI captions for X videos via Mistral Voxtral — no server needed.
// @author       Hermes
// @match        https://x.com/*
// @match        https://twitter.com/*
// @grant        GM_xmlhttpRequest
// @grant        GM_notification
// @connect      api.mistral.ai
// @connect      video.twimg.com
// @downloadURL  https://raw.githubusercontent.com/Esashiero/x-captions/main/x-loader.user.js
// @updateURL    https://raw.githubusercontent.com/Esashiero/x-captions/main/x-loader.user.js
// @run-at       document-start
// ==/UserScript==

(function() {
    'use strict';

    var MISTRAL_API_KEY = '';  // <-- PASTE YOUR MISTRAL API KEY HERE
    var MISTRAL_MODEL = 'voxtral-mini-latest';

    var captionsData = null;
    var captionInterval = null;
    var isTranscribing = false;

    // ── HLS URL CATCHER ───────────────────────────────────
    var _capturedAudioUrl = null;

    (function() {
        var origOpen = XMLHttpRequest.prototype.open;
        XMLHttpRequest.prototype.open = function(method, url) {
            if (typeof url === 'string' && url.indexOf('/pu/pl/mp4a/') > 0 && url.indexOf('.m3u8') > 0) {
                _capturedAudioUrl = url;
            }
            return origOpen.apply(this, arguments);
        };
    })();

    // ── GM_XHR WRAPPER ──────────────────────────────────
    function gmFetch(url) {
        return new Promise(function(resolve, reject) {
            GM_xmlhttpRequest({
                method: 'GET',
                url: url,
                onload: function(r) { resolve(r.responseText); },
                onerror: function() { reject(new Error('net')); },
                ontimeout: function() { reject(new Error('timeout')); }
            });
        });
    }

    function gmFetchBin(url) {
        return new Promise(function(resolve, reject) {
            GM_xmlhttpRequest({
                method: 'GET',
                url: url,
                onload: function(r) { resolve(r.response); },
                onerror: function() { reject(new Error('net')); },
                ontimeout: function() { reject(new Error('timeout')); }
            });
        });
    }

    // ── SVG ICON ──────────────────────────────────────────
    var CC_SVG = '<svg viewBox="0 0 24 24" aria-hidden="true" class="r-4qtqp9 r-yyyyoo r-dnmrzs r-bnwqim r-lrvibr r-m6rgpd r-z80fyv r-19wmn03"><g><path d="M9.007 8.785c1.26 0 2.075.53 2.62 1.29l-1.207.935c-.306-.42-.799-.695-1.357-.695-.93 0-1.684.754-1.684 1.684 0 .93.755 1.684 1.684 1.684.578 0 1.087-.292 1.39-.735l1.22.87c-.582.802-1.367 1.394-2.736 1.394h-.002l-.002.003c-1.766 0-3.187-1.35-3.187-3.187s1.421-3.186 3.187-3.186zm7.602 0c1.26 0 2.075.53 2.62 1.29l-1.207.935c-.306-.42-.799-.695-1.357-.695-.93 0-1.684.754-1.684 1.684 0 .93.755 1.684 1.684 1.684.578 0 1.087-.292 1.39-.735l1.22.87c-.582.802-1.367 1.394-2.736 1.394h-.002l-.002.003c-1.766 0-3.187-1.35-3.187-3.187s1.421-3.186 3.187-3.186z"></path></g></svg>';

    // ── UI ────────────────────────────────────────────────
    function makeBtn() {
        var w = document.createElement('div');
        w.className = 'css-175oi2r';
        w.setAttribute('data-x-feature', 'ai-captions');
        w.setAttribute('data-active', 'false');

        var b = document.createElement('button');
        b.setAttribute('aria-label', 'AI Captions');
        b.setAttribute('role', 'button');
        b.className = 'css-175oi2r r-sdzlij r-1phboty r-rs99b7 r-lrvibr r-2yi16 r-1qi8awa r-1loqt21 r-o7ynqc r-6416eg r-1ny4l3l';
        b.style.cssText = 'background-color:rgba(0,0,0,0);border-color:rgba(0,0,0,0);opacity:0.5;';
        var n = document.createElement('div');
        n.dir = 'ltr';
        n.className = 'css-146c3p1 r-qvutc0 r-1qd0xha r-q4m81j r-a023e6 r-rjixqe r-b88u0q r-1awozwy r-6koalj r-18u37iz r-16y2uox r-bcqeeo r-1777fci';
        n.style.cssText = 'color:rgb(255,255,255);';
        n.innerHTML = CC_SVG;
        b.appendChild(n);
        b.addEventListener('click', function(e) { e.stopPropagation(); handleClick(w); });
        w.appendChild(b);
        return w;
    }

    function getPlayer() {
        var w = document.querySelector('[data-x-feature="ai-captions"][data-active="true"]') || document.querySelector('[data-x-feature="ai-captions"]');
        if (!w) return document.querySelector('[data-testid="videoPlayer"]');
        return w.closest('[data-testid="videoPlayer"]');
    }

    function handleClick(w) {
        var active = w.getAttribute('data-active') === 'true';
        if (active) { deactivate(w); return; }
        activate(w);
    }

    function deactivate(w) {
        w.setAttribute('data-active', 'false');
        var btn = w.querySelector('button');
        if (btn) btn.style.opacity = '0.5';
        hideOverlay();
        if (captionInterval) { clearInterval(captionInterval); captionInterval = null; }
    }

    function activate(w) {
        if (!MISTRAL_API_KEY) { showStatus('Set MISTRAL_API_KEY in script'); return; }
        w.setAttribute('data-active', 'true');
        var btn = w.querySelector('button');
        if (btn) btn.style.opacity = '1';
        if (captionsData) { startCaptionDisplay(); return; }
        if (isTranscribing) { showStatus('Transcribing...'); return; }
        showStatus('Starting...');
        isTranscribing = true;
        startTranscription();
    }

    // ── CORE: FIND & PROCESS HLS AUDIO ──────────────────
    function findAudioPlaylist() {
        if (_capturedAudioUrl) return _capturedAudioUrl;
        try {
            var entries = performance.getEntriesByType('resource');
            for (var i = entries.length - 1; i >= 0; i--) {
                var name = entries[i].name;
                if (name.indexOf('/pu/pl/mp4a/') > 0 && name.indexOf('.m3u8') > 0) return name;
            }
        } catch(e) {}
        return null;
    }

    function startTranscription() {
        var audioUrl = findAudioPlaylist();
        if (!audioUrl) {
            showStatus('Waiting for video...');
            setTimeout(function() {
                audioUrl = findAudioPlaylist();
                if (!audioUrl) { showStatus('No video data'); isTranscribing = false; return; }
                processAudioPlaylist(audioUrl);
            }, 3000);
            return;
        }
        processAudioPlaylist(audioUrl);
    }

    function processAudioPlaylist(audioUrl) {
        showStatus('Loading audio...');
        gmFetch(audioUrl).then(function(text) {
            var initUrl = null;
            var segmentUrls = [];
            for (var i = 0; i < text.length; i++) {
                var line = text.split('\n')[i];
                if (!line) continue;
                line = line.trim();
                if (line.indexOf('#EXT-X-MAP:URI="') >= 0) {
                    var m = line.match(/URI="([^"]+)"/);
                    if (m) initUrl = m[1].indexOf('http') === 0 ? m[1] : ('https://video.twimg.com' + m[1]);
                }
                if (line.indexOf('/ext_tw_video/') === 0 && line.indexOf('.m4s') > 0) {
                    segmentUrls.push('https://video.twimg.com' + line);
                }
            }
            if (!initUrl || segmentUrls.length === 0) { showStatus('Empty audio'); isTranscribing = false; return; }
            console.log('[X] Init:', initUrl, '| Segs:', segmentUrls.length);
            showStatus('Downloading (' + segmentUrls.length + ' segs)...');
            downloadAudio(initUrl, segmentUrls);
        }).catch(function(err) {
            console.error('[X] Fetch fail:', err);
            showStatus('Net error');
            isTranscribing = false;
        });
    }

    function downloadAudio(initUrl, segmentUrls) {
        var all = [initUrl].concat(segmentUrls);
        var blobs = [], done = 0, total = 0;
        all.forEach(function(url, idx) {
            gmFetchBin(url).then(function(bin) {
                var len = bin.length;
                var bytes = new Uint8Array(len);
                for (var i = 0; i < len; i++) bytes[i] = bin.charCodeAt(i) & 0xFF;
                blobs[idx] = new Blob([bytes], {type: 'audio/mp4'});
                total += blobs[idx].size;
                done++;
                if (done % 10 === 0 || done === all.length) showStatus('Download (' + done + '/' + all.length + ', ' + Math.round(total/1024) + 'KB)');
                if (done === all.length) sendToMistral(blobs, total);
            }).catch(function(err) {
                console.error('[X] Seg ' + idx + ' fail:', err);
                showStatus('Dl error');
                isTranscribing = false;
            });
        });
    }

    // ── MISTRAL UPLOAD ────────────────────────────────────
    function sendToMistral(blobs, total) {
        showStatus('Transcribing... (' + Math.round(total/1024) + 'KB)');
        var blob = new Blob(blobs, {type: 'audio/mp4'});
        var boundary = '----FB' + Math.random().toString(36).slice(2);
        var p = [];
        p.push('--' + boundary);
        p.push('Content-Disposition: form-data; name="model"');
        p.push(''); p.push(MISTRAL_MODEL);
        p.push('--' + boundary);
        p.push('Content-Disposition: form-data; name="timestamp_granularities"');
        p.push(''); p.push('segment');
        p.push('--' + boundary);
        p.push('Content-Disposition: form-data; name="file"; filename="audio.mp4"');
        p.push('Content-Type: audio/mp4');
        p.push('');
        var textBefore = p.join('\r\n') + '\r\n';
        var textAfter = '\r\n--' + boundary + '--';
        var enc = new TextEncoder();
        var pref = enc.encode(textBefore);
        var suff = enc.encode(textAfter);
        blob.arrayBuffer().then(function(buf) {
            var totalLen = pref.length + buf.byteLength + suff.length;
            var c = new Uint8Array(totalLen);
            c.set(pref, 0);
            c.set(new Uint8Array(buf), pref.length);
            c.set(suff, pref.length + buf.byteLength);
            var str = '';
            for (var i = 0; i < c.length; i++) str += String.fromCharCode(c[i]);
            GM_xmlhttpRequest({
                method: 'POST',
                url: 'https://api.mistral.ai/v1/audio/transcriptions',
                headers: {'Authorization': 'Bearer ' + MISTRAL_API_KEY, 'Content-Type': 'multipart/form-data; boundary=' + boundary},
                data: str,
                onload: function(r) {
                    isTranscribing = false;
                    try {
                        var resp = JSON.parse(r.responseText);
                        if (resp.segments && resp.segments.length > 0) {
                            captionsData = resp.segments.map(function(s) { return {start: s.start, end: s.end, text: (s.text||'').trim()}; });
                            console.log('[X] OK', captionsData.length, 'segs');
                            hideOverlay(); startCaptionDisplay();
                        } else if (resp.text) {
                            captionsData = [{start: 0, end: (resp.usage ? resp.usage.prompt_audio_seconds : 60) || 60, text: resp.text.trim()}];
                            hideOverlay(); startCaptionDisplay();
                        } else { showStatus('API Error'); console.error('[X]', resp); }
                    } catch(e) { showStatus('Parse'); console.error('[X] Parse:', e); }
                },
                onerror: function() { isTranscribing = false; showStatus('API down'); },
                ontimeout: function() { isTranscribing = false; showStatus('API timeout'); }
            });
        });
    }

    // ── CAPTION DISPLAY ──────────────────────────────────
    function showStatus(msg) {
        var p = getPlayer(); if (!p) return;
        removeOverlays(p);
        var o = document.createElement('div');
        o.setAttribute('data-x-feature', 'caption-overlay');
        o.style.cssText = 'position:absolute;bottom:60px;left:0;right:0;text-align:center;padding:8px 16px;z-index:9999;pointer-events:none;';
        var t = document.createElement('div');
        t.style.cssText = 'display:inline-block;background:rgba(0,0,0,0.75);color:white;padding:6px 14px;border-radius:6px;font:14px/1.4 sans-serif;max-width:80%;';
        t.textContent = msg; o.appendChild(t);
        var vc = p.querySelector('[data-testid="videoComponent"]');
        if (vc) vc.appendChild(o);
    }

    function startCaptionDisplay() {
        if (!captionsData) return;
        var p = getPlayer(); if (!p) return;
        var v = p.querySelector('video'); if (!v) return;
        removeOverlays(p);
        var o = document.createElement('div');
        o.setAttribute('data-x-feature', 'caption-overlay');
        o.style.cssText = 'position:absolute;bottom:60px;left:0;right:0;text-align:center;padding:8px 16px;z-index:9999;pointer-events:none;';
        var t = document.createElement('div');
        t.id = 'caption-text';
        t.style.cssText = 'display:inline-block;background:rgba(0,0,0,0.85);color:white;padding:8px 16px;border-radius:6px;font:15px/1.5 sans-serif;max-width:85%;text-align:center;';
        var c = v.currentTime;
        for (var i = 0; i < captionsData.length; i++) {
            if (c >= captionsData[i].start && c < captionsData[i].end) { t.textContent = captionsData[i].text; break; }
        }
        o.appendChild(t);
        var vc = p.querySelector('[data-testid="videoComponent"]');
        if (vc) vc.appendChild(o);
        if (captionInterval) clearInterval(captionInterval);
        captionInterval = setInterval(function() {
            if (!v) return;
            var c = v.currentTime, f = '';
            for (var i = 0; i < captionsData.length; i++) { if (c >= captionsData[i].start && c < captionsData[i].end) { f = captionsData[i].text; break; } }
            t.textContent = f;
        }, 200);
    }

    function hideOverlay() { var p = getPlayer(); if (p) removeOverlays(p); }
    function removeOverlays(p) { var els = p.querySelectorAll('[data-x-feature="caption-overlay"]'); for (var i = 0; i < els.length; i++) els[i].remove(); }

    // ── PLAYER DETECTION ─────────────────────────────────
    function processPlayer(player) {
        if (player.querySelector('[data-x-feature="ai-captions"]')) return;
        if (player.querySelector('[data-testid="captions"]')) return;
        var unmute = player.querySelector('[aria-label="Unmute"]') || player.querySelector('[aria-label="Mute"]');
        if (!unmute) return;
        var btnWrapper = unmute.parentElement;
        if (!btnWrapper) return;
        var cb = btnWrapper.parentElement;
        if (!cb) return;
        cb.insertBefore(makeBtn(), btnWrapper);
    }

    var retries = 0;
    function tryProcess() {
        var ps = document.querySelectorAll('[data-testid="videoPlayer"]');
        var ok = false;
        for (var i = 0; i < ps.length; i++) { processPlayer(ps[i]); if (ps[i].querySelector('[data-x-feature="ai-captions"]')) ok = true; }
        if (!ok && retries < 60) { retries++; setTimeout(tryProcess, 500); }
    }

    function init() {
        console.log('[X] Start v3.2');
        new MutationObserver(function() {
            var ps = document.querySelectorAll('[data-testid="videoPlayer"]');
            for (var i = 0; i < ps.length; i++) processPlayer(ps[i]);
        }).observe(document.body, { childList: true, subtree: true });
        tryProcess();
    }

    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init);
    else init();
})();
