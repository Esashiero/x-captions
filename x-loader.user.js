...
(function() {
    ...
    var _audioM3u8 = null;   // /pu/pl/mp4a/...
    var _masterM3u8 = null;  // /pu/pl/...m3u8 (no avc1/mp4a)
    var _videoM3u8 = null;   // /pu/pl/avc1/...
    ...
    function pick3u8() {
        ...
        try {
            ...
            for (var i=ee.length-1; i>=0; i--) {
                var n = ee[i].name;
                if (n.indexOf('/pu/pl/mp4a/') > -1 && n.indexOf('.m3u8') > -1) return n;
            }
            for (var i=ee.length-1; i>=0; i--) {
                var n = ee[i].name;
                if (n.indexOf('/pu/pl/') > -1 && n.indexOf('.m3u8') > -1 && n.indexOf('avc1') < 0) return n;
            }
            for (var i=ee.length-1; i>=0; i--) {
                var n = ee[i].name;
                if (n.indexOf('/pu/pl/avc1/') > -1 && n.indexOf('.m3u8') > -1) return n;
            }
        } catch(e) {}
        return null;
    }
    ...
    function mk() {
        ...
        b.setAttribute('aria-label','AI Captions'); b.setAttribute('role','button');
    ...
    function on(w) {
        if (w.getAttribute('data-on')==='1') {
            ...
            hide();if(intv){clearInterval(intv);intv=null;}return;
        }
    ...
    function load(url) {
        ...
        get(url).then(function(txt) {
            var lines = txt.split('\n');
            console.log('[X] lines:', lines.length);

            // Master playlist?
            if (url.indexOf('mp4a/') > -1) {
                // Already an audio playlist, parse directly
                return parseAudio(txt, url);
            }

            // Check if master
            var isMaster = lines.some(function(l){return l.indexOf('#EXT-X-STREAM-INF')===0;});
            if (isMaster) {
                console.log('[X] master, finding audio...');
                ...
                console.log('[X] audio url:', au.slice(0,100));
                return load(au);
            }

            // Variant playlist (might be video). Check if audio by looking for aud/ in init
            return parseAudio(txt, url);
        }).catch(function(e){console.error('[X] err:',e);sts('Net err');busy=false;});
    }

    function parseAudio(txt, url) {
        var lines = txt.split('\n');
        var init=null, segs=[];
        lines.forEach(function(l){
            l=l.trim();
        ...
        // Verify it's an audio playlist — init should have "aud/" in path
        var isAudio = init.indexOf('/pu/aud/') > -1;
        if (!isAudio) {
            console.log('[X] not audio init, trying to find real audio playlist via performance...');
            // Try to find audio URL
            if (_audioM3u8) { console.log('[X] found audio URL:', _audioM3u8.slice(0,100)); return load(_audioM3u8); }
            // Last attempt: look in performance entries
            try {
                var ee = performance.getEntriesByType('resource');
                for(var i=ee.length-1;i>=0;i--){
                    if(ee[i].name.indexOf('/pu/pl/mp4a/')>-1&&ee[i].name.indexOf('.m3u8')>-1){
                        console.log('[X] perf audio:', ee[i].name.slice(0,100));
                        return load(ee[i].name);
                    }
                }
            } catch(e){}
        ...
        console.log('[X] audio init:', init.slice(0,80));
    ...
    function send(blobs,total) {
        ...
        var b='----FB'+Math.random().toString(36).slice(2), p=[];
        ...
        var hd=p.join('\r\n')+'\r\n', tl='\r\n--'+b+'--';
        var enc=new TextEncoder(), hb=enc.encode(hd), tb=enc.encode(tl);
                ...
                onload:function(r){
                    ...
                    try{
                        ...
                        if(j.segments&&j.segments.length){
                            ...
                            console.log('[X] OK',caps.length,'segs');hide();showCaps();
                        }else if(j.text){
    ...
    function init(){
        console.log('[X] v3.4');
        new MutationObserver(function(){var ps=document.querySelectorAll('[data-testid="videoPlayer"]');for(var i=0;i<ps.length;i++)inj(ps[i]);}).observe(document.body,{childList:true,subtree:true});
        tryGo();
    }
