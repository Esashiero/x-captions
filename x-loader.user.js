...
(function() {
        ...
        var obs = new MutationObserver(function(ms) {
            for (var mi=0; mi<ms.length; mi++) {
                ...
                for (var ai=0; ai<added.length; ai++) {
                    ...
                    // Update text to "Captions"
                    var spans = ci.querySelectorAll('span');
                    for (var si=0; si<spans.length; si++) {
                        spans[si].textContent = 'Captions';
                    }
    ...
    function init(){console.log('[X] v6.5');watchMenu();new MutationObserver(function(){var ps=document.querySelectorAll('[data-testid="videoPlayer"]');for(var i=0;i<ps.length;i++)inj(ps[i]);}).observe(document.body,{childList:true,subtree:true});tryGo();}
